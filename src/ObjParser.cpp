...
void ObjParser::parseObjects(vector<Triangle *> &objects, Material mat, vector<cv::Vec3f> &vertexes, double scale = 1.0)
{
    ...
    while (obj)
    {
        ...
        {
            obj >> x_c >> y_c >> z_c;
            // Apply scale factor to the vertex coordinates
            vertexes.emplace_back(x_c * scale, y_c * scale, z_c * scale);
        }
